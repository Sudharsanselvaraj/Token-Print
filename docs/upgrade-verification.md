...
- Overview → Layer → Operation transitions and manual-camera preservation passed in a real browser.
...
Limits: the clean-install run was on macOS; Linux CPU installation is covered by the new release workflow and must be green before release. Other browser/GPU combinations are unverified. Browser GPT-2 exposes final logits, not attention/hidden-state instrumentation. Experiment files are durable downloads; the recent-results list is session-only. No claim is made that different hardware/runtime versions produce bit-identical outputs.
